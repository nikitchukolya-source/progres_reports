...
# Файл Service Account від Google
...
def get_managers_data(sheet):
    ...
    for row in rows:
        ...
        # На цьому рядку закінчується список менеджерів
...
def build_message(managers):
    ...
    for manager in managers:
        ...
        # Для підсумку
        try:
            total_efforts += int(str(efforts).replace(" ", "").replace("—", "0"))
        except ValueError:
            pass

        try:
            total_offers += int(str(offers).replace(" ", "").replace("—", "0"))
        except ValueError:
            pass
